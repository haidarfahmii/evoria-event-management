...
export default function PointWidget({
  originalPrice,
  onApplyPoints,
}: PointWidgetProps) {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2 text-gray-400 text-sm py-2">
        <ImSpinner8 className="animate-spin" /> Loading points...
      </div>
    );
  }
    ...
    return (
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 flex items-start gap-2 text-gray-500">
        ...
        <div className="text-xs">
          ...
          <p>
            Minimum {new Intl.NumberFormat("id-ID").format(STEP)} points to
            redeem.
          </p>
        </div>
      </div>
    ...
    <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-3">
      ...
      <div className="flex justify-between items-center">
        ...
        <span className="text-xs font-semibold text-amber-700 ">
          Balance: {new Intl.NumberFormat("id-ID").format(totalPoints)}
        </span>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-between items-center">
          <span className="text-xs text-amber-700 font-medium">
            Used {new Intl.NumberFormat("id-ID").format(redeemAmount)} Point
          </span>
          <span className="text-sm font-bold text-amber-900">
            - {formatRupiah(redeemAmount)}
          </span>
        </div>

        {/* Info Text jika maxed out oleh harga tiket */}
        {maxRedeemableByPrice < maxRedeemableByBalance &&
          ...
          maxRedeemable > 0 && (
            <div className="flex items-start gap-1 text-[10px] text-amber-700 bg-amber-100/50 p-1.5 rounded">
              ...
              Maximum use of points is limited by the total ticket price.
            </div>
          )}
      </div>
    </div>
