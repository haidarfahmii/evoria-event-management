...
export default function ReviewSection({ eventId }: ReviewSectionProps) {
    ...
    const checkPurchase = async () => {
      ...
      try {
        ...
        const found = transactions.find(
          (t: any) => t.eventId === eventId && t.status === "DONE"
        );
  ...
  const renderFormSection = () => {
    if (isLoading || isCheckingEligibility || status === "loading") {
      return (
        <div className="h-20 bg-gray-100 rounded-2xl animate-pulse mb-6"></div>
      );
    }
      ...
      return (
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6 text-center mb-6">
          ...
          <p className="text-blue-700 text-sm">
            Organizers cannot write reviews for events. Please switch to a
            Customer account.
          </p>
        </div>
      );
    ...
    if (!hasPurchased) {
      return (
        <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-6 text-center mb-6">
          <h3 className="text-yellow-800 font-bold text-sm mb-1">
            Ticket Required
          </h3>
          <p className="text-yellow-700 text-sm">
            You need to purchase a ticket and complete the event to write a
            review.
          </p>
        </div>
      );
